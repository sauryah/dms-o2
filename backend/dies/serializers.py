...
class ArchardWearInputSerializer(serializers.Serializer):
    tonnage = serializers.ListField(
        child=serializers.FloatField(min_value=0.0001),
        min_length=2,
        help_text="List of cumulative production tonnages (tonnes)",
    )
    wear_um = serializers.ListField(
        child=serializers.FloatField(min_value=0.0001),
        min_length=2,
        help_text="List of measured die bore wear readings (micrometers)",
    )
    tolerance_um = serializers.FloatField(
        default=8.0,
        min_value=0.1,
        required=False,
        help_text="Maximum allowable die bore wear tolerance ceiling (micrometers)",
    )

    def validate(self, attrs):
        if len(attrs['tonnage']) != len(attrs['wear_um']):
            raise serializers.ValidationError("Tonnage and wear_um arrays must have equal lengths.")
        return attrs


class JohnsonCookInputSerializer(serializers.Serializer):
    material = serializers.ChoiceField(
        choices=['copper', 'high_carbon_steel', 'aluminum', 'brass'],
        help_text="Target wire drawing material alloy",
    )
    strain = serializers.FloatField(
        min_value=0.001,
        help_text="True plastic drawing strain (ln(A0/Af))",
    )
    strain_rate = serializers.FloatField(
        default=1.0,
        min_value=0.0001,
        required=False,
        help_text="Effective drawing strain rate (s^-1)",
    )
    temperature_c = serializers.FloatField(
        default=20.0,
        min_value=-50.0,
        required=False,
        help_text="Workpiece starting temperature (Celsius)",
    )


class WeibullReliabilityInputSerializer(serializers.Serializer):
    lifetimes = serializers.ListField(
        child=serializers.FloatField(min_value=0.0001),
        min_length=3,
        help_text="Historical die service lifetimes before recutting/scrapping (tonnes or hours)",
    )
