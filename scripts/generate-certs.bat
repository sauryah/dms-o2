...
if exist "%DYNAMIC_YML%" (
    ...
    if not errorlevel 1 (
        ...
        if exist "%SCRIPTS_DIR%client-install-template.bat" (
            powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "(Get-Content '%~dp0client-install-template.bat') -replace '{{CLIENT_NAME}}', 'lihas.dms' -replace '{{LAN_IP}}', '%LAN_IP%' | Set-Content '%CERTS_DIR%\client-lihas.dms-install.bat'"
        )
        if exist "%SCRIPTS_DIR%client-install-template.sh" (
            powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "(Get-Content '%~dp0client-install-template.sh') -replace '{{CLIENT_NAME}}', 'lihas.dms' -replace '{{LAN_IP}}', '%LAN_IP%' | Set-Content '%CERTS_DIR%\client-lihas.dms-install.sh'"
        )
        if exist "%SCRIPTS_DIR%client-instructions-template.txt" (
            powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "(Get-Content '%~dp0client-instructions-template.txt') -replace '{{CLIENT_NAME}}', 'lihas.dms' -replace '{{LAN_IP}}', '%LAN_IP%' | Set-Content '%CERTS_DIR%\client-lihas.dms-INSTRUCTIONS.txt'"
        )
    )
)
