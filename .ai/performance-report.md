...
*   **Verification**: Added custom routes `CELERY_TASK_ROUTES` in [settings.py](file:///home/sahil/Desktop/Projects/dms-o2/backend/dms/settings.py) to route heavy CSV imports and backups to the `heavy` queue, leaving search syncs on the `default` queue. Spawned a secondary `heavy-worker` container service in Compose configurations and Supervisor.
...
*   **Verification**: Cached wear forecast responses in Redis for 24 hours inside `wear_prediction` endpoint in [views.py](file:///home/sahil/Desktop/Projects/dms-o2/backend/dies/views.py). Set up event-driven cache invalidation hooks inside die/dimension post-save signals in [signals.py](file:///home/sahil/Desktop/Projects/dms-o2/backend/dies/signals.py).
