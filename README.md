...
| **Cannot connect/access from phone or external device** | Host IP not allowed in Django, Windows Network Category is Public, or Firewall blocking Docker Backend. | DMS auto-detects the server's LAN IP at startup and adds it to `ALLOWED_HOSTS`. If this fails, manually add your IP to `DJANGO_ALLOWED_HOSTS` in `.env` (e.g., `DJANGO_ALLOWED_HOSTS=localhost,127.0.0.1,django,192.168.1.15`) and restart: `docker compose up -d django`. On Windows, set your network profile to **Private** in admin PowerShell: `Set-NetConnectionProfile -InterfaceAlias Wi-Fi -NetworkCategory Private`. Update Docker's inbound rules to apply to all profiles. |
