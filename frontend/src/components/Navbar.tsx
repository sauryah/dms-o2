...
export function Navbar() {
  ...
  return (
    <nav className="border-b border-slate-900 bg-slate-950/85 backdrop-blur-md sticky top-0 z-50 shadow-sm">
          ...
          <div className="flex items-center space-x-4">
            {username ? (
              <div className="hidden sm:flex items-center space-x-4">
                <div className="text-right">
                  ...
                  <span className="block text-[10px] text-slate-500 font-mono tracking-wider uppercase">{role}</span>
                </div>
                <Link
                  to="/settings"
            ...
            {username && (
              <div className="relative" ref={notificationRef}>
                <button
                  ...
                  className="relative p-2 text-slate-400 hover:text-white hover:bg-slate-900/50 rounded-xl transition-all duration-300 border border-transparent hover:border-slate-800 cursor-pointer focus-ring"
                  aria-label="Notification Center"
                >
                  <Bell className="h-5 w-5" />
                  {unreadCount > 0 && (
                    <span className="absolute top-1.5 right-1.5 flex h-4 w-4 items-center justify-center rounded-full bg-rose-500 text-[9px] font-bold text-white ring-2 ring-slate-950 shadow-lg shadow-rose-950/20" aria-label={`${unreadCount} unread notifications`}>
                      {unreadCount}
                    </span>
                  )}
                </button>
                    ...
                    <div className="max-h-72 overflow-y-auto divide-y divide-slate-800/40">
                      {notifications.length === 0 ? (
                        <div className="py-10 text-center px-4">
                          <Bell className="h-8 w-8 text-slate-600 mx-auto mb-2.5" />
            ...
            <div className="flex sm:hidden">
              <button
                ...
                className="inline-flex items-center justify-center p-2 rounded-lg text-slate-400 hover:text-white hover:bg-slate-900 focus-ring transition-all duration-200"
      ...
      {/* Mobile Menu Panel */}
      {isOpen && (
        <div id="mobile-menu" className="sm:hidden border-t border-slate-800/80 bg-slate-950/95 backdrop-blur-lg px-4 pt-2 pb-4 space-y-2.5 animate-menuSlideDown">
          <Link
            to="/"
          ...
          {isAuthorizedForTools && (
            <div className="px-3.5 py-2">
              ...
              <div className="space-y-1.5 pl-3 mt-1.5 border-l border-slate-700">
                {(role === 'ROOT' || (authorizedTools || []).includes('sizing-calculator')) && (
                  <Link
                    to="/calculator"
          ...
          {username ? (
            <div className="pt-4 mt-2 border-t border-slate-800 flex flex-col space-y-3 px-3.5">
              <div className="flex flex-col">
                ...
                <span className="text-[10px] text-slate-500 font-mono tracking-wider uppercase">{role}</span>
              </div>
              <Link
                to="/settings"
